...
int min(double a, double b){ //mínim entre a i b
	if(a>=b){return b;}
	else{return a;}
}
int max(double a, double b){ //màxim entre a i b
	if(a<=b){return b;}
	else{return a;}
}
int ompleMinMax(Node* pare, int jugadaPropera, double beta){ //Donat el pare, omple tots els valors i retorna pare->valor, &&&& jugadaPropera = 0, beta = 1e10
	int i, j;
	double alpha = -1e10;
	...
	if(pare->n_fills == 0){
		...
		for(i=0; i<pare->n_fills; i++){
			if(pare->fills[i]->n_fills == 0){
				...
				alpha = max(alpha, pare->fills[i]->valor);
				if(alpha>=beta){
					break;
				}
			}
			else{
				if(quatreRatlla(pare->fills[i]->tauler) == -4){ //Si 4 ratlla (ORD) ja no posa valors a l'arbre de baix i valora segons el nivell (es pensa com node fulla)
					pare->fills[i]->valor = funcioHeuristica(pare->fills[i], jugadaPropera+2);
					alpha = max(alpha, pare->fills[i]->valor);
					if(alpha>=beta){
						break;
					...
					for(j=0; j<pare->fills[i]->n_fills; j++){
						if(pare->fills[i]->fills[j]->n_fills == 0){
							...
							beta = min(beta, pare->fills[i]->fills[j]->valor);
							if(beta<=alpha){
								break;
							}
						}
						else{
							if(quatreRatlla(pare->fills[i]->fills[j]->tauler) == 4){ //Si 4 ratlla (PERS) no posa valors a l'arbre de baix i valora segons nivell (com fulla)
								pare->fills[i]->fills[j]->valor = funcioHeuristica(pare->fills[i]->fills[j], jugadaPropera+2);
								beta = min(beta, pare->fills[i]->fills[j]->valor);
								if(beta<=alpha){
									break;
								...
								pare->fills[i]->fills[j]->valor = ompleMinMax(pare->fills[i]->fills[j], jugadaPropera+2, beta);
								beta = min(beta, pare->fills[i]->fills[j]->valor); //minim valor dels pare->fills[i]->fills[j]
								if(beta<=alpha){
									break;
								}
							}
					...
					pare->fills[i]->valor = beta;
					beta = 1e10;
					alpha = max(alpha, pare->fills[i]->valor);
				}
			}
		} // final bucle i
		pare->valor = alpha; //Màxim dels valors dels i fills de pare que no s'han tallat
	}
...
int miniMax(Node* pare){ //Donat el pare, torna la columna on millor tirar segons el mètode del minimax
	...
	max = ompleMinMax(pare, 0, 1e10); // Omplo els "valor", que és l'única variable que falta omplir i determino el valor màxim
	//recorreArbre(pare, 1);
	//printf("\nMaxim = %d", max);
